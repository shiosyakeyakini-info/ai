...
``` json
{
	...
	"aichatEmojiUpdateIntervalMinutes": "絵文字データの更新間隔(分)。aichatとemoji-reactモジュールで使用する絵文字データを指定した間隔で更新する。デフォルトは1440分(24時間)",
...
``` json
{
	...
	"aichatGroundingWithGoogleSearchAlwaysEnabled": "aichatでGoogle検索を利用したグラウンディングを常に行う場合 true を入れる (無効にする場合は false(いずれも二重引用符(\")は不要))",
	"aichatEmojiUpdateIntervalMinutes": "絵文字データの更新間隔(分)。aichatとemoji-reactモジュールで使用する絵文字データを指定した間隔で更新する。デフォルトは1440分(24時間)",
	"reactedAiChatTargetCategories": "aichat版リアクションで使用するカテゴリリスト",
	"reactedAiChatTargetEmojisRatio": "aichat版リアクションのカテゴリリストで、トークン数削減のためにランダムで使用する絵文字を絞る場合の比率",
	"reactedAiChatProbability": "aichat版リアクションを発生させる確率",
	"mecab": "/usr/bin/mecab",
